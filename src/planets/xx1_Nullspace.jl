...
function my_nullspace(A::AbstractVecOrMat; atol::Real = 0.0, rtol::Real = (min(size(A, 1), size(A, 2))*eps(real(float(oneunit(eltype(A))))))*iszero(atol))
...
function KernelFromFile(fnS::String,isRight::Bool, nmets::Int, nrxns::Int)
...
N = length(files)
for i = 1:N 
    push!(chunks[mod(i-1,numTasks)+1],i)
end 

for i in chunks[taskNum] 
    fn = saveDir*lpad(i,3,"0")*".csv" 
    if isfile(fn)
        continue 
    end 
    
    A = KernelFromFile(files[i], isRight, nmets, nrxns) 
    if isRight 
