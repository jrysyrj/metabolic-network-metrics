...
fileList = dir([resultsPath filesep 'geneGroup*_consistent.mat']);
...
for j = 1:length(fileNames)
    fn = fileNames{j}
    load(fn);
    sol = linearMOMA(WT_model,model,'max');
    isViable{j}(1) = sol.f > 0;
end

save([resultsPath filesep 'Model_viability_linearMOMA'],'isViable','-v7.3');
